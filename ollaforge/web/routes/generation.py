...
async def start_generation(
    ...
    background_tasks.add_task(_run_generation_task, task_id, request)
...
async def _run_generation_task(task_id: str, request: GenerationRequest):
    ...
    try:
        ...
        generation_service.update_task(task_id, status="running", total=request.count)
        ...
        async def async_progress_callback(completed: int, total: int):
            generation_service.update_task(task_id, progress=completed, total=total)
            await ws_manager.emit_progress(
                task_id=task_id,
                progress=completed,
                total=total,
        ...
        generation_service.update_task(task_id, status="failed", error=error_msg)
